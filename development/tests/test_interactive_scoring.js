// Test Interactive Scoring with User Prompts
...
// Store user responses for missing requirements
...
// Function to prompt user for missing requirement
function promptForRequirement(requirement, jobTitle) {
  return new Promise((resolve) => {
    ...
    console.log('❓ Do you have this requirement? (Y/N):');
    
    rl.question('', (answer) => {
      const hasRequirement = answer.toLowerCase().startsWith('y');
...
async function testInteractiveScoring() {
  console.log('🧪 TESTING INTERACTIVE SCORING WITH USER PROMPTS');
  ...
  try {
    ...
    if (missingRequirements.length > 0) {
      ...
      console.log('\n🎯 Step 3: Interactive prompting for missing requirements...');
      
      for (const requirement of missingRequirements) {
        await promptForRequirement(requirement, testJob.title);
      }
...
// Run the test
testInteractiveScoring().catch(console.error); 