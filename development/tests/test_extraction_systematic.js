...
class ExtractionSystematicTest {
  constructor() {
    this.results = {
      totalJobs: 0,
      successfulJobs: 0,
      ...
      accuracy: {
        ...
        jobUrls: { found: 0, total: 0 }
      },
      errors: []
    };
    
    this.testUrl = 'https://www.seek.com.au/project-jobs/in-Dandenong-VIC-3175?daterange=7&distance=25&sortmode=ListedDate';
  }

  async initialize() {
    console.log('🔍 EXTRACTION SYSTEMATIC TEST STARTING');
    console.log('=' .repeat(60));
    
    this.results.timing.startTime = Date.now();
    
    console.log(`🎯 Testing extraction with top 5 jobs from: ${this.testUrl}`);
    console.log(`📋 Will create lists of extracted data for analysis`);
  }

  async testExtraction() {
    console.log('\n🚀 TESTING EXTRACTION PHASE');
    ...
    try {
      ...
      console.log(`📊 Found ${jobData.length} jobs for extraction`);
      ...
      console.error('❌ Extraction test error:', error.message);
      await browser.close();
      throw error;
    }
  }
  ...
  async analyzeResults(jobs) {
    ...
    jobs.forEach((job, index) => {
      ...
      // Track individual data points
      if (job.title) {
        this.results.accuracy.jobTitles.found++;
        this.results.extractedData.jobTitles.push(job.title);
      }
      if (job.company) {
        this.results.accuracy.companies.found++;
        this.results.extractedData.companies.push(job.company);
      }
      if (job.location) {
        this.results.accuracy.locations.found++;
        this.results.extractedData.locations.push(job.location);
      }
      if (job.postedTime) {
        this.results.accuracy.postedTimes.found++;
        this.results.extractedData.postedTimes.push(job.postedTime);
      }
      if (job.jobUrl) {
        this.results.accuracy.jobUrls.found++;
        this.results.extractedData.jobUrls.push(job.jobUrl);
      }
      ...
      this.results.accuracy.jobUrls.total++;
      ...
      console.log(`${status} Job ${index + 1}: ${job.title}`);
  ...
  printSummary() {
    ...
    console.log(`   Job URLs: ${this.results.accuracy.jobUrls.found}/${this.results.accuracy.jobUrls.total} (${((this.results.accuracy.jobUrls.found / this.results.accuracy.jobUrls.total) * 100).toFixed(1)}%)`);
    
    // Extracted Data Lists
    console.log(`\n📋 EXTRACTED DATA LISTS:`);
    console.log(`   Job Titles (${this.results.extractedData.jobTitles.length}):`);
    this.results.extractedData.jobTitles.forEach((title, index) => {
      console.log(`     ${index + 1}. ${title}`);
    });
    
    console.log(`\n   Companies (${this.results.extractedData.companies.length}):`);
    this.results.extractedData.companies.forEach((company, index) => {
      console.log(`     ${index + 1}. ${company}`);
    });
    
    console.log(`\n   Locations (${this.results.extractedData.locations.length}):`);
    this.results.extractedData.locations.forEach((location, index) => {
      console.log(`     ${index + 1}. ${location}`);
    });
    
    console.log(`\n   Posted Times (${this.results.extractedData.postedTimes.length}):`);
    this.results.extractedData.postedTimes.forEach((time, index) => {
      console.log(`     ${index + 1}. ${time}`);
    });
    
    console.log(`\n   Job URLs (${this.results.extractedData.jobUrls.length}):`);
    this.results.extractedData.jobUrls.forEach((url, index) => {
    ...
    if (this.results.accuracy.postedTimes.found / this.results.accuracy.postedTimes.total < 0.8) {
      console.log(`   ⚠️ Posted time extraction below 80% - consider improving time extraction logic`);
    }
    if (this.results.accuracy.jobUrls.found / this.results.accuracy.jobUrls.total < 0.8) {
      console.log(`   ⚠️ Job URL extraction below 80% - consider improving URL extraction`);
    }
    if (this.results.timing.averagePerJob > 3000) {
      console.log(`   ⚠️ Average job processing time > 3s - consider optimizing extraction speed`);
    }
  ...
  async run() {
    try {
      ...
      const jobs = await this.testExtraction();
...
// Run the test
const test = new ExtractionSystematicTest();
test.run(); 